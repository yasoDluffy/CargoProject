...
namespace CargoProject
{
    public class CargoTrain : CargoVehicle
    {
        ...
        public CargoTrain(string driver, double maxWeight, double maxVolume, Port currentPort)
        : base()
        {
            Driver = driver;
            MaxWeight = maxWeight;
        ...
        public void TravelToNextPort()
        {
            if (NextPort == null)
            {
                Console.WriteLine("Next port not set.");
                return;
            }

            Console.WriteLine($"Traveling to {NextPort.PortName}.");
            CurrentPort.UnloadVehicle(this);
            NextPort.DockVehicle(this);
            CurrentPort = NextPort;
            NextPort = null; // נסיעה הסתיימה
        }
